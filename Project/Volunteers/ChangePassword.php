...
if (isset($_POST["btn_submit"])) {
    ...
    $selbc = "SELECT * FROM tbl_volunteers WHERE volunteers_password='$currentpwd' AND volunteers_id='" . $_SESSION["vid"] . "'";
    ...
    if ($data = $resbc->fetch_assoc()) {
        ...
        if ($newpwd == $confirmpwd) {
            ...
            if (preg_match('/^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/', $newpwd)) {
                ...
                $upQry = "UPDATE tbl_volunteers SET volunteers_password='$newpwd' WHERE volunteers_id='" . $_SESSION["vid"] . "'";
                if ($con->query($upQry)) {
