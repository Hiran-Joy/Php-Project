...
if (isset($_POST["btn_submit"])) {
    ...
    $selbc = "SELECT * FROM tbl_admin WHERE admin_password='$currentpwd' AND admin_id='" . $_SESSION["aid"] . "'";
    ...
    if ($data = $resbc->fetch_assoc()) {
        ...
        if ($newpwd == $confirmpwd) {
            ...
            if (preg_match('/^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/', $newpwd)) {
                ...
                $upQry = "UPDATE tbl_admin SET admin_password='$newpwd' WHERE admin_id='" . $_SESSION["aid"] . "'";
                if ($con->query($upQry)) {
