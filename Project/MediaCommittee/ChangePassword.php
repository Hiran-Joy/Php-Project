...
if (isset($_POST["btn_submit"])) {
    ...
    $selbc = "SELECT * FROM tbl_media_committee WHERE mc_password='$currentpwd' AND mc_id='" . $_SESSION["mid"] . "'";
    ...
    if ($data = $resbc->fetch_assoc()) {
        ...
        if ($newpwd == $confirmpwd) {
            ...
            if (preg_match('/^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/', $newpwd)) {
                ...
                $upQry = "UPDATE tbl_media_committee SET mc_password='$newpwd' WHERE mc_id='" . $_SESSION["mid"] . "'";
                if ($con->query($upQry)) {
