...
if (isset($_POST["btn_submit"])) {
    ...
    $selbc = "SELECT * FROM tbl_program_committee WHERE pc_password='$currentpwd' AND pc_id='" . $_SESSION["pgid"] . "'";
    ...
    if ($data = $resbc->fetch_assoc()) {
        ...
        if ($newpwd == $confirmpwd) {
            ...
            if (preg_match('/^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/', $newpwd)) {
                ...
                $upQry = "UPDATE tbl_program_committee SET pc_password='$newpwd' WHERE pc_id='" . $_SESSION["pgid"] . "'";
                if ($con->query($upQry)) {
