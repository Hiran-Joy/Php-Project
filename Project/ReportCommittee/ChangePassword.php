...
if (isset($_POST["btn_submit"])) {
    ...
    $selbc = "SELECT * FROM tbl_report_committee WHERE rc_password='$currentpwd' AND rc_id='" . $_SESSION["rid"] . "'";
    ...
    if ($data = $resbc->fetch_assoc()) {
        ...
        if ($newpwd == $confirmpwd) {
            ...
            if (preg_match('/^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/', $newpwd)) {
                ...
                $upQry = "UPDATE tbl_report_committee SET rc_password='$newpwd' WHERE rc_id='" . $_SESSION["rid"] . "'";
                if ($con->query($upQry)) {
